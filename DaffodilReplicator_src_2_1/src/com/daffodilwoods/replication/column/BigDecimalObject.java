...
public class BigDecimalObject extends AbstractColumnObject
{
     ...
     * @param sqlType0
     */
    public BigDecimalObject(int sqlType0,AbstractDataBaseHandler abstractDBHandler0 )
    {
        sqlType = sqlType0;
                                ...
                                int index) throws SQLException
    {
        ...
        {
            pst.setBigDecimal(index, BigDecimal.valueOf(Long.parseLong(value)));
        }
    }
        ...
        IOException, SQLException
    {
        try
        {
          if (!encodedCols.contains(col.toUpperCase())) {
              bw.write(getObject(rs, index).toString() );
//            bw.write("<![CDATA[" + getObject(rs, index).toString() + "]]>");
          } else {
            bw.write(EncoderDecoder.escapeUnicodeString1(getObject(rs, index).
                            toString(), true));
...
//                toString(), true) + "]]>");
    ...
    private void write(Writer bw, Object rowValue,ArrayList encodedCols,String col) throws IOException
    {
        try
        {
            if(!encodedCols.contains(col.toUpperCase())) {
            bw.write("<![CDATA[" + rowValue.toString() + "]]>");
            } else {
              bw.write("<![CDATA[" +
                            ...
                            String columnName,ArrayList encodedCols) throws SQLException, IOException
    {
        ...
        {
            write(bw, newObject,encodedCols,columnName);
            if (oldObject != null)
            {
