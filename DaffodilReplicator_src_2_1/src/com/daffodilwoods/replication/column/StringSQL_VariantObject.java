...
public class StringSQL_VariantObject extends AbstractColumnObject
{
    ...
    public StringSQL_VariantObject(int sqlType0,AbstractDataBaseHandler abstractDBHandler0)
    {
        sqlType = sqlType0;
                                ...
                                int index) throws SQLException
    {
        ...
        {
            if (value.startsWith("start") && value.indexOf("length") != -1)
            {
                int lenghtIndex = value.indexOf("length");
                int start = Integer.parseInt(value.substring(5, lenghtIndex));
                int length = Integer.parseInt(value.substring(lenghtIndex + 6));
                byte[] bytes = new RBlob(start, length).getBytes(1, length);
